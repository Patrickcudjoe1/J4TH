    ...
    return (
        <div className="w-full bg-white text-black min-h-screen">
            <div className="max-w-6xl mx-auto flex flex-col md:flex-row gap-6 px-4 md:px-8 py-6">
                ...
                {/* Gallery / Carousel (left on desktop) */}
                <div className="md:w-1/2 md:sticky md:top-6">
                    ...
                    >
                        ...
                        <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-2">
                            {images.map((_, i) => (
                                <span key={i} className={`w-2 h-2 rounded-full ${i === index ? 'bg-black' : 'bg-black/20'}`} />
                            ))}
                        </div>
                    </div>
                </div>
                ...
                <div className="md:w-1/2 flex flex-col gap-4">
                    ...
                    <div className="mt-2">
                        <div className="md:hidden">
                            <button onClick={() => setExpanded((v) => !v)} className="text-sm text-gray-700">
                                {expanded ? 'Hide' : 'Read More'}
                            </button>
                            {expanded && <div className="mt-2 text-sm text-gray-700">Long product description, benefits and materials go here. Minimal, focused copy.</div>}
                        </div>
